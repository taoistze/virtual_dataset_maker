...
//#pragma omp parallel
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)
		{
			...
			//单位为MM，需要进行单位变换
			/*vector.x = mesh->mVertices[i].x * 0.001;
			vector.y = mesh->mVertices[i].y * 0.001;
			vector.z = mesh->mVertices[i].z * 0.001;
			vertex.Position = vector;*/

			//单位为英寸
			vector.x = mesh->mVertices[i].x*0.0254;
			vector.y = mesh->mVertices[i].y * 0.0254;
			vector.z = mesh->mVertices[i].z * 0.025;
			vertex.Position = vector;
			// normals
